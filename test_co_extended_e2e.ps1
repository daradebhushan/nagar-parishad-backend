...
Write-Host "--- EXTENDED E2E CHIEF OFFICER (ADMIN) API TEST SUITE ---`n"
...
Write-Host "`n>> 5. CO CRUD TASKS TEST (CREATE, READ, UPDATE, DELETE) <<"
$taskBody = @{
    title       = "CO Extended E2E Task"
    description = "Full CRUD Delete Test"
...
try {
    ...
    Invoke-RestMethod -Uri "$baseUrl/tasks/$($newTask.data.id)" -Method Delete -Headers $headersCo
    Write-Host "SUCCESS: CO successfully DELETED Task (ID: $($newTask.data.id))."
}
catch {
    Write-Host "FAILED on Task CRUD: $_"
}

Write-Host "`n>> 6. CO COMPLAINTS MANAGEMENT TEST (UPDATE STATUS) <<"
try {
    ...
    if ($complaints.data.content.Count -gt 0) {
        ...
        Write-Host "CO Attempting to ACCEPT complaint..."
        $statusBody = "ACCEPTED"
        $statusRes = Invoke-RestMethod -Uri "$baseUrl/admin/complaints/$compId/status" -Method Put -Body $statusBody -ContentType "application/json" -Headers $headersCo
        Write-Host "SUCCESS: CO Updated Complaint Status to '$($statusRes.data.status)'"
    }
}
catch {
